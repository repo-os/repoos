...
updated_at: "2026-08-24T17:51:08Z"
---
## Problem
...
1. **The "did not become ready" timeout is the one-size-fits-all error.** `doStart` (preview.ts:324) waits a fixed `HEALTH_TIMEOUT_MS` (10s) for `/api/health`, and on timeout reports only "did not become ready". It does NOT distinguish:
   - a **build** failure/slowness during `ensureFreshBuild` (preview.ts:154 — this can take up to `BUILD_TIMEOUT_MS` = 240s `spawnSync`, and the error *is* returned there, which is at least semi-actionable),
...
- **Immediate loading indicator.** When `startPreview` (and `stopPreview`) is clicked, show a spinner/disabled state *immediately* (right now `previewBusy` is already tracked — use it, or a dedicated `previewStarting` ref — to render a spinner inside the Start/Stop button and/or a "Starting preview…" text next to the button). The button should disable while in flight) so the ~10s wait has visible feedback.
- **Show the actionable error inline** near the preview control (not only a transient toast), so the user sees "what failed and what to do" and can retry. Consider surfacing `result.error` from the POST body (already thrown through `repo.onError` via `api()`).
