...
- `agents.ts` — the AgentRunner: spawns coding agents in task worktrees, streams
  ...
  and self-heals board state when the agent exits. Its Copilot CLI driver uses
  JSONL output and explicit, narrow write/shell permissions; it never uses the
  CLI's unrestricted permission switches.
