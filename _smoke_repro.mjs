...
const server = await startServer({
  root: "/Users/nick/code/nick/repoos-worktrees/feat/let-s-add-a-new-agent-to-build-your-team",
  host: "127.0.0.1",
  port: 0,
});
...
try {
  ...
  page.on("requestfailed", (req) => {
    failed.push(`REQFAIL ${req.url()} ${req.failure()?.errorText}`);
  });
  ...
  console.log("HAS MUSTACHE:", bodyText.includes("{{"));
  await browser.close();
} finally {
  await server.close();
}
