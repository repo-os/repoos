...
describe("frontmatter quote escaping", () => {
  ...
  it("parses the canonical single-escape stored form to plain quotes", () => {
    const out = parseDocument(doc(String.raw`"Replace \"branch\" terminology with \"worktree\""`));
    expect(out.data.title).toBe('Replace "branch" terminology with "worktree"');
  });
  ...
  it("keeps serialize(parse(y)) stable for the canonical escaped form", () => {
    ...
    const reserialized = serializeDocument(parsed.data, parsed.body, ["id", "title", "type"]);
    expect(reserialized).toContain(String.raw`title: "Make default \"Assign To\" set to AI"`);
  });
  ...
  it('drops one accumulated escape level per parse (0051 stored \\\\" form)', () => {
    ...
    expect(out.data.title).toBe(String.raw`Replace \"branch\" terminology with \"worktree\"`);
  });

  it('drops one accumulated escape level per parse (0005 stored \\\\\\\\" form)', () => {
    const out = parseDocument(doc(String.raw`"Make default \\\\"Assign To\\\\" set to AI"`));
    expect(out.data.title).toBe(String.raw`Make default \\\"Assign To\\\" set to AI`);
  });
