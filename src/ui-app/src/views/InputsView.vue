...
function loadFiltersFromStorage(): void {
  try {
    ...
    selected.value = new Set(stored ? JSON.parse(stored) : ["new", "reviewing"]);
  } catch {
    selected.value = new Set(["new", "reviewing"]);
  }
}
...
async function move(i: Input, status: string): Promise<void> {
  i.status = status as Input["status"];
  await repo.updateInput(i.id, status);
}
function nextStatus(status: Input["status"]): Input["status"] | null {
  return status === "new" ? "reviewing" : status === "reviewing" ? "processed" : null;
}
function moveNext(i: Input): void {
  const next = nextStatus(i.status);
  if (next) void move(i, next);
}
function statusLabel(status: Input["status"]): string {
      ...
      ><DialogOverlay /><DialogContent
        ...
        <div v-if="activeInput" class="drawer-body input-detail">
          <div class="detail-status">
            <Select
              :model-value="activeInput.status"
              @update:model-value="(v) => move(activeInput!, v ?? '')"
              ><SelectTrigger class="detail-select"
                ><span class="select-label"
                  ><span class="state-dot" :class="'state-' + activeInput.status"></span
                  >{{ statusLabel(activeInput.status) }}</span
                ></SelectTrigger
              ><SelectContent position="popper"
                ><SelectViewport class="min-w-[var(--radix-select-trigger-width)]"
                  ><SelectItem v-for="s in statuses" :key="s.id" :value="s.id"
                    ><span class="state-dot" :class="'state-' + s.id"></span
                    >{{ s.label }}</SelectItem
                  ></SelectViewport
                ></SelectContent
              ></Select
            ><Button
              v-if="nextStatus(activeInput.status)"
              variant="outline"
              class="move-next"
              @click="moveNext(activeInput)"
              ><ArrowRight class="size-3.5" /><span
                class="state-dot"
                :class="'state-' + nextStatus(activeInput.status)"
              ></span>
