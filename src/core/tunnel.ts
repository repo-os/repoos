...
export function renderCloudflaredConfig(cfg: TunnelConfig, credentialsFile: string): string {
  const lines: string[] = [];
  lines.push(
    "# Generated by repoos tunnel — the machine registry (~/.config/repoos/tunnel-apps.toml) is",
    "# the source of truth for what's ingress-routed; each repo's repoos.toml [tunnel] owns its",
    "# own apps within it. See docs/tunnel-registry.md.",
  );
