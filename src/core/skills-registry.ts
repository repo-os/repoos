...
async function request<T>(url: string): Promise<T> {
  const response = await fetch(url, { headers: { Accept: "application/json", "User-Agent": "RepoOS skills discovery" }, signal: AbortSignal.timeout(12_000) });
...
export async function curatedRegistrySkills(): Promise<RegistrySkill[]> {
  ...
  return results.flat().filter((skill) => !seen.has(skill.id) && (seen.add(skill.id), true)).slice(0, 30);
}

export async function searchRegistrySkills(query: string): Promise<RegistrySkill[]> {
  const q = query.trim();
  if (q.length < 2) return curatedRegistrySkills();
  const body = await request<{ skills: Array<{ id: string; skillId?: string; name: string; installs: number; source: string }> }>(`${SEARCH_API}?q=${encodeURIComponent(q)}&limit=30`);
...
export async function registryDetail(id: string): Promise<RegistryDetail> {
  ...
  const tree = await request<{ tree: Array<{ path: string; type: string }> }>(`${GITHUB_API}/repos/${source}/git/trees/${commit.sha}?recursive=1`);
  const skillMd = tree.tree.find((entry) => entry.type === "blob" && entry.path === `skills/${slug}/SKILL.md`)
  ...
  const paths = tree.tree.filter((entry) => entry.type === "blob" && entry.path.startsWith(directory)).map((entry) => entry.path);
  const files = await Promise.all(paths.map(async (path) => ({
    path: path.slice(directory.length),
    contents: await fetchText(`https://raw.githubusercontent.com/${source}/${commit.sha}/${path}`),
  })));
  const hash = createHash("sha256").update(files.map((file) => `${file.path}\0${file.contents}`).join("\0")).digest("hex");
...
export async function registryAudit(id: string): Promise<RegistryAudit | null> {
  try {
    ...
    const data = await request<Record<string, Record<string, { risk?: string; alerts?: number }>>>(`${AUDIT_API}?source=${encodeURIComponent(source)}&skills=${encodeURIComponent(slug)}`);
    return {
      audits: Object.entries(data[slug] ?? {}).map(([provider, result]) => ({
        provider,
        status: result.risk === "safe" ? "pass" : result.risk === "high" || result.risk === "critical" ? "fail" : "warn",
...
async function fetchText(url: string): Promise<string> {
  const response = await fetch(url, { headers: { "User-Agent": "RepoOS skills discovery" }, signal: AbortSignal.timeout(12_000) });
  if (!response.ok) throw new Error(`Skill source request failed (${response.status})`);
