...
describe("installRegistrySkill", () => {
  it("vendors files and locks the registry hash", () => {
    ...
    expect(readFileSync(join(root, "skills", "design", "references", "tokens.md"), "utf8")).toBe(
      "tokens",
    );
    expect(JSON.parse(readFileSync(join(root, "skills.lock.json"), "utf8"))).toMatchObject({
      design: { id: "acme/skills/design", hash: "abc123" },
    });
  });
  ...
  it("rejects an unsafe payload before it writes anything", () => {
    ...
    roots.push(root);
    expect(() =>
      installRegistrySkill(config(root), {
        id: "acme/skills/design",
        source: "acme/skills",
        slug: "design",
        installs: 1,
