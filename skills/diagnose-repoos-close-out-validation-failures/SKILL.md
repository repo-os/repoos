...
2. **Check for a reload race.** If a reload was in flight when close-out began, the failure is environmental: a close-out must not start while a replacement is spawning or handing over. Fix: wait for the reload to settle before retrying.
...
5. **Inspect `mergeBranch` in `src/core/git.ts`** auto-resolve path. Confirm lock-contention signals — `index.lock` or `Unable to create … .lock` — are classified as **retryable**, not `non-retryable`.
...
7. **Verify by hand.** Run the merge/resolve steps manually to isolate environmental causes from logic causes. Do not retry repeatedly while a reload or lock holder is still active.
...
3. **Check for task file drift before merging.** Run `git diff main...HEAD --name-only | grep '^work/'` in the worktree. Any `work/` file other than the task's own `.md` is drift — restore it from `main` before merging:
